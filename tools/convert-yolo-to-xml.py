...
folder_holding_yolo_files = "."
#  input("Enter the path to the yolo files: ").replace("'", "").strip()
yolo_class_list_file =  ".\\classes.txt"
# input("Enter the path to the file that has the yolo classes (typically classes.txt): ").strip()
...
for each_yolo_file in os.listdir(folder_holding_yolo_files):
  if each_yolo_file.endswith("txt"):
    ...
    if os.path.exists(each_yolo_file.replace('txt', 'tiff')):
      image_name = each_yolo_file.replace('txt', 'tiff')
